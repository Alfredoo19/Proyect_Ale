...
app.post("/api/casos", upload.single("archivo"), (req, res) => {
  try {
    const { promovente, demandado, juzgado, no_expediente, municipio } = req.body;
    if (!promovente || !demandado || !juzgado || !municipio) {
      return res.status(400).json({ error: "Faltan campos requeridos" });
    }
    const archivo_path = req.file ? "/uploads/" + path.basename(req.file.path) : null;
    ...
    `);
    const info = stmt.run({ promovente, demandado, juzgado, no_expediente: no_expediente || null, municipio, archivo_path });
...
app.get("/api/casos", (req, res) => {
  try {
    ...
    if (!no_expediente) return res.status(400).json({ error: "Se requiere no_expediente" });
    const rows = db.prepare("SELECT * FROM casos WHERE no_expediente = ? ORDER BY id DESC").all(no_expediente);
...
app.post("/api/expedientes", (req, res) => {
  try {
    const { nombre_completo, fecha, hora, etiqueta, asunto } = req.body || {};
